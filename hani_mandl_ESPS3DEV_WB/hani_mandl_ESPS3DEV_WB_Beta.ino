/*
  ...
  2023-11 Wolfgang Birkfellner
                                - Pinbelegung für Waveshare ESP32-S3-WROOM-1-N8R8 und Waveshare 1.5" OLED Moldule geändert
                                - DIB-> OIB geändert
...
#define MAXIMALGEWICHT 2000     // Maximales Gewicht

// Ansteuerung der Waage
#define SCALE_READS 5      // Parameter für hx711 Library. Messwert wird aus der Anzahl gemittelt
...
const int outputSW = 36;

// Servo
const int servo_pin = 21;

// 3x Schalter Ein 1 - Aus - Ein 2
const int switch_betrieb_pin = 16;
const int switch_vcc_pin     = 17;     // <- Vcc 
const int switch_setup_pin   = 18;

// Taster 
const int button_start_vcc_pin =  15;  // <- Vcc
const int button_start_pin     =  7;
const int button_stop_vcc_pin  =  6;  // <- Vcc
const int button_stop_pin      =  5;
...
const char *GlasTypArray[3] = { "OIB", "TOF", "DEE"};//OIB = OesterreichischerImkerBund-Glas DEE= DeepTwist-Glas TOF= TwistOff-Glas //JB
...
void getPreferences(void) {
    ...
    i = 0;
    int ResetGewichte[] = {30,120,170,230,450};
    int ResetGlasTyp[] = {1,1,1,1,1};
    while( i < 5) {
...
//    Serial.print(" servo_aktiv ");     Serial.print(servo_aktiv);
  ...
  if(servo_aktiv == 1) {
    ...
    u8g2.drawBox  (0, 50, progressbar, 40 );
  } 
  else
  {
    if( autokorrektur == 1 ){
...
void print_credits() {
  ...
  u8g2.setCursor(0, 20);    u8g2.print("Idee: M. Vasterling");
  u8g2.setCursor(0, 33);    u8g2.print("Code: M. Vasterling, M.");
  u8g2.setCursor(0, 46);    u8g2.print("Wetzel, C. Gruber, A.");
  u8g2.setCursor(0, 59);    u8g2.print("Holzhammer, M. Junker,");
  u8g2.setCursor(0, 72);    u8g2.print("J. Kuder, J. Bruker");
  u8g2.setCursor(0, 85);    u8g2.print("Conversion ESP32-DEV:");
  u8g2.setCursor(0, 98);    u8g2.print("W. Birkfellner");
  u8g2.sendBuffer();
}
...
void print_logo() {
  ...
  u8g2.setCursor(70, 74);    u8g2.print("ESP32-DEV");
  u8g2.sendBuffer();
}
